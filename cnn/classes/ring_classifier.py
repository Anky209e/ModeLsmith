...
def predict(path):
    ...
    pred_model.load_state_dict(torch.load("./models/ring_classifier.pth", map_location=torch.device("cpu")))
    
    img_cls = ['No Ring', 'Ring']

    transform = ToTensor()
    img_tensor = transform(img)
    img_tensor = img_tensor[:3]
    
    img_tensor = torch.reshape(img_tensor, (1,3,101,101))
